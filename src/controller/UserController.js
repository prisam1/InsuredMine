...
const {checkInputsPresent,isValidObjectId,isValid,isValidName,isValidMobile,validateEmail,isValidzip} =require("../valid/valid")
...
const Createuser= async function(req,res)
{
  try{
       if(!checkInputsPresent(req.body))
        ...
        if(!isValid(state))
            return res.status(400).send({ status: false, message: "Atate is required" })

        if(!isValidName(state))
            return res.status(400).send({ status: false, message: "Enter a Valid State" })
  
        if(!isValid(city))
          return res.status(400).send({ status: false, message: "City Number is required" })

        if(!isValidName(city))
          return res.status(400).send({ status: false, message: "Enter a Valid City" })

        if(!isValid(zip))
          return res.status(400).send({ status: false, message: "Zip is required" })  

        if(!isValidzip(zip))
          return res.status(400).send({ status: false, message: "Enter a Valid zip" })   
...
if(zip || zip=="")
{
  ...
  let update=await UserModel.findByIdAndUpdate({_id:id},{...req.body},{new:true})
...
const deleteUser = async function (req, res) {
  try {
      ...
      let customer = await UserModel.findOneAndUpdate({ _id: UserId, isDeleted: false }, { isDeleted: true, deletedAt: new Date() })
      if (customer) {
          return res.status(200).send({ status: true, message: "User deleted successfuly" })
      }
      else {
