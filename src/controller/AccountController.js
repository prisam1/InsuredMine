  ...
  try{
       if(!checkInputsPresent(req.body))
        ...
        if(!isValid(account_name))
          return res.status(400).send({ status: false, message: "Account name is required" })
...
{  if(!isValid(premium_amount))
  ...
  let update=await AccountModel.findByIdAndUpdate({_id:id},{...req.body},{new:true})
...
const deleteAccount = async function (req, res) {
  try {
      ...
      if (!mongoose.Types.ObjectId.isValid(AccountId)) {
          return res.status(400).send({ status: false, message: "Invalid Account ID" })
      }

      let Account = await AccountModel.findOneAndUpdate({ _id: AccountId, isDeleted: false }, { isDeleted: true, deletedAt: new Date() })
      if (Account) {
          return res.status(200).send({ status: true, message: "Account deleted successfuly" })
      }
      else {
